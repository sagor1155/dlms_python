...
class GXDLMSReader(PyQt5.QtCore.QObject):
    
    displaymsg = pyqtSignal(str)
    
    def __init__(self, client, media, trace, parent=None):
        super(GXDLMSReader, self).__init__(parent)
        self.iec = False
    ...
    def initializeConnection(self):
        ...
        if self.iec and isinstance(self.media, GXSerial):
            print("initializeConnection : iec mode")
            p = ReceiveParameters()
            p.allData = False
            p.eop = '\n'
            p.waitTime = self.waitTime
